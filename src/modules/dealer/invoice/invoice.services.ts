...
export const generateInvoiceFromSaleId = async (saleId: string) => {
  ...
  const sdaShare = Number(sale.sda_commission!) || 0
  const dealerShare = Number(sale.dealer_commission!) || 0
  const tdsAmount = Number(sale.tds_amount!) || 0
  ...
  const invoiceData = {
    ...
    sacCode: "998729",
    qty: 1,
    rate: Math.floor(Number(sale.total_amount)),
    cgst: "0.00",
    sgst: "0.00",
    igst: 56.44,
    amount: Math.floor(Number(sale.sda_commission)),
    amountInWords: "Indian Rupees " + toWords(Number(sale.sda_commission)) + " Only",
    taxableValue: Math.floor(Number(sale.sda_commission)) - 56.44,
    cgstTotal: "0.00",
    sgstTotal: "0.00",
    igstTotal: 56.44,
    commission: Math.floor(Number(sale.dealer_commission)),
    tds: Math.floor(Number(sale.tds_amount)),
    totalInvoiceValue: Math.floor(sdaShare + tdsAmount),

...
export const createEmployee = async (req: Request, res: Response<ApiResponse<any>>): Promise<Response> => {
    try {
      const {
        first_name,
        last_name,
    ...
    const { data: employee, error } = await db.from("employees").insert([
        {
          employee_id,
      ...
      const financeHead = await db.from("employees").select("id").eq("role_id", "finance_head").single();
      if (financeHead.data) {
        await db.from("notifications").insert([
          {
            recipient_id: financeHead.data.id,
      ...
      const performedBy = req.user?.id ? req.user.id : "system"; 
      ...
      await db.from("audit_logs").insert([
        {
          ...
          remarks: `HR submitted an onboarding request for ${first_name} ${last_name}. Waiting for Finance Head approval.`,
        }
      ]);
  
      return res.status(201).json({ success: true, message: "Employee onboarding request submitted successfully.", data: employee });

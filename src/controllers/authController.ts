...
export const loginEmployee = async (
  ...
  try {
    ...
    const { data: employee, error: employeeError } = await db
      .from("employees")
      .select(
        "id, employee_id, first_name, last_name, password, department:department_id(id, name)"
      )
    ...
    if (employeeError) {
      ...
      return res.status(500).json({
        success: false,
        message: "Database error fetching employee.",
    ...
    if (!employee.password) {
      ...
      return res
        .status(400)
        .json({ success: false, message: "Employee password not set. Contact admin." })
    }
    ...
    if (rolesError) {
      ...
      return res.status(500).json({
        success: false,
        message: "Database error fetching roles.",
    ...
    if (permissionsError) {
      ...
      return res.status(500).json({
        success: false,
        message: "Database error fetching permissions.",
...
export const getAllEmployees = async (req: Request, res: Response<any>): Promise<any> => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, message: "Employees fetched successfully.", data: employees })
  } catch (error) {
    return res
      .status(500)

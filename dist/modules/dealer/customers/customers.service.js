...
exports.getAllCustomersService = exports.createCustomerService = void 0;
const db_1 = require("../../../config/db");
const bcrypt_1 = __importDefault(require("bcrypt"));
const generateDealerId_1 = require("../../../utils/generateDealerId");
const generatePolicyNumber = (state, oemCode, serial, planCode) => {
    ...
    const stateCode = generateDealerId_1.STATE_CODE_MAP[normalizedState];
    return `${stateCode}${oemCode}${serialStr}${year}${planCode}`;
};
const createCustomerService = async (input, dealerEmployeeId) => {
    const { customer, vehicle, rsa_plan } = input;
    let customer_id = null;
    let vehicle_id = null;
    try {
        // check if customer already exists and the buy the same plan send error
        const { data: existingCustomer } = await db_1.db
            .from("customers")
            .select("id")
            .eq("phone", customer.phone)
            .single();
        if (existingCustomer?.id) {
            const { data: customerCurrentPlan } = await db_1.db
                .from("rsa_plan_sales")
        ...
        const { data: employee, error: empErr } = await db_1.db
            .from("dealer_employees")
            .select("dealer_id, id")
            .eq("id", dealerEmployeeId)
            .single();
        if (empErr || !employee)
            throw new Error("Dealer employee not found");
        const { data: cust, error: customerErr } = await db_1.db
            .from("customers")
            .insert({
            ...customer,
        ...
        const { data: veh, error: vehicleErr } = await db_1.db
            .from("vehicles")
        ...
        const { data: plan } = await db_1.db
            .from("rsa_plans")
            .select("name")
            .eq("id", rsa_plan.plan_id)
            .single();
        const planCodeMap = { standard: "S", premium: "P", elite: "E" };
        const planCode = planCodeMap[plan?.name.toLowerCase()] || "S";
        ...
        const { error: planErr } = await db_1.db
            .from("rsa_plan_sales")
            .insert([rsaPayload]);
        if (planErr)
            throw new Error("RSA Plan insert failed: " + planErr.message);
        const hashedPassword = await bcrypt_1.default.hash("Welcome@123", 10);
        const loginPayload = {
            customer_id,
        ...
        const { error: loginErr } = await db_1.db
            .from("customer_logins")
            .insert([loginPayload]);
        if (loginErr)
            throw new Error("Login insert failed: " + loginErr.message);
        return {
            status: 201,
            success: true,
        ...
        if (vehicle_id)
            await db_1.db.from("vehicles").delete().eq("id", vehicle_id);
        if (customer_id)
            await db_1.db.from("customers").delete().eq("id", customer_id);
        return {
            status: 500,
            success: false,
...
const getAllCustomersService = async (id) => {
    try {
        const { data, error } = await db_1.db
            ...
            .eq("dealer_id", id);
        if (error)
            throw new Error("Customers fetch failed: " + error.message);
        return data;
    }
    catch (error) {
        return {
